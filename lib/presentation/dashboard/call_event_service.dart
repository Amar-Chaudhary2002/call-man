...
class CallEventService {
  ...
  Future<void> init() async {
    ...
    if (!phoneGranted) {
      ...
      await _showFallbackDialog("Permission Error", "Phone permission not granted. Please enable it in settings.");
      return;
    }
    ...
    try {
      _subscription = PhoneState.stream.listen(
        ...
        onError: (error) {
          ...
          _showFallbackDialog("Call Detection Error", "Error monitoring phone state: $error");
        },
        cancelOnError: false,
      );
      ...
      await _showFallbackDialog("Initialization Error", "Failed to start call monitoring: $e");
    }
  }
  ...
  Future<void> _processPhoneStateChange(PhoneState event) async {
    ...
    try {
      ...
      await _showFallbackDialog("Call Processing Error", "Error handling call state: $e");
    }
  }
  ...
  Future<bool> _showOverlay(String title, String subtitle, String callState) async {
  ...
  Future<void> _showFallbackDialog(String title, String message, {bool autoClose = false}) async {
  ...
  Future<void> testOverlay() async {
    ...
    final overlayShown = await _showOverlay("Test Overlay", "This is a test overlay", "test");
    if (!overlayShown) {
      await _showFallbackDialog("Test Result", "Overlay failed to show. This dialog confirms call detection is working!");
    }
  }
